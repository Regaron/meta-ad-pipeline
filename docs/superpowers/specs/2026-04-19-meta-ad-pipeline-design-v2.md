...
| 6. Register tools in MCP server | ✅ Committed (`eb8cb0c`) — exposes `scrape_url` + `render_creative` | **Amend `tools/mcp_server.py` in a new commit:** add `view_brand_reference` import + tool registration. Add a `VIEW_BRAND_REFERENCE_TOOL = f"mcp__{SERVER_NAME}__view_brand_reference"` constant alongside the existing two. |
